...
def get_all_fund_codes(file_path):
    ...
    for encoding in encodings_to_try:
        try:
            # 尝试读取文件，注意 'utf-8-sig' 会自动移除 BOM 字符
...
async def fetch_fund_info(fund_code, session, semaphore):
    ...
    async with semaphore:
        try:
            ...
            # 调试：记录响应内容的前100字符（已清理BOM）
            print(f"    -> 基金 {fund_code} 响应内容（前100字符，已清理BOM）: {js_text[:100]}")
            ...
            # 4. 提取核心 JSON 数据块 (用于获取成立日期、估值等非简单变量)
            data_main = {}
            # 优先匹配 apidata={...}
            ...
            if json_match:
                ...
                try:
                    # 尝试将 JS Object Literal 转换为 JSON 并加载
                    # 替换单引号为双引号
                    ...
                    # 如果解析失败，可能是结构太复杂或格式有误，跳过
                    pass
            
            # 提取 Data_fund_info (成立日期/经理等信息通常在这里)
            data_info = {}
            ...
            print(f"    -> 基金 {fund_code} 基本信息抓取成功 (名称: {result['名称']})")
